pipeline {
    ...
    environment {
        ...
        HOST_PORT = '3000'
        CONTAINER_PORT = '3000'
        REMOTE_SERVER = 'ubuntu@13.219.77.142'
    }
    ...
    stages {
        stage('Test SSH Login') {
            steps {
                sshagent(['my-ssh-key']) {
                    sh "ssh -o StrictHostKeyChecking=no ${REMOTE_SERVER} 'echo SSH login successful'"
                }
            }
        }
        ...
        stage('Remove Old Backup Image') {
            steps {
                sshagent(['my-ssh-key']) {
                    script {
                        def oldImageId = sh(script: "ssh -o StrictHostKeyChecking=no ${REMOTE_SERVER} docker images -q ${OLD_IMAGE_TAG}", returnStdout: true).trim()
                        if (oldImageId) {
                            echo "Removing old image: ${OLD_IMAGE_TAG}"
                            sh "ssh -o StrictHostKeyChecking=no ${REMOTE_SERVER} docker rmi ${OLD_IMAGE_TAG}"
                        } else {
        ...
        stage('Tag Current Latest as Old') {
            steps {
                sshagent(['my-ssh-key']) {
                    script {
                        def currentImageId = sh(script: "ssh -o StrictHostKeyChecking=no ${REMOTE_SERVER} docker images -q ${LATEST_IMAGE_TAG}", returnStdout: true).trim()
                        if (currentImageId) {
                            echo "Tagging current latest as old_image"
                            sh "ssh -o StrictHostKeyChecking=no ${REMOTE_SERVER} docker tag ${LATEST_IMAGE_TAG} ${OLD_IMAGE_TAG}"
                        } else {
        ...
        stage('Build New Docker Image') {
            steps {
                sshagent(['my-ssh-key']) {
                    sh "ssh -o StrictHostKeyChecking=no ${REMOTE_SERVER} docker build -t ${LATEST_IMAGE_TAG} -f ${DOCKERFILE_PATH} ${WORKDIR}"
                }
            }
        }
        ...
        stage('Restart Container') {
            steps {
                sshagent(['my-ssh-key']) {
                    script {
                        def containerId = sh(script: "ssh -o StrictHostKeyChecking=no ${REMOTE_SERVER} docker ps -q --filter name=${CONTAINER_NAME}", returnStdout: true).trim()
                        if (containerId) {
                            echo "Stopping and removing running container: ${CONTAINER_NAME}"
                            sh "ssh -o StrictHostKeyChecking=no ${REMOTE_SERVER} docker stop ${containerId}"
                            sh "ssh -o StrictHostKeyChecking=no ${REMOTE_SERVER} docker rm ${containerId}"
                        } else {
                            echo "No container is currently running."
                        }

                        echo "Running new container from latest image..."
                        sh "ssh -o StrictHostKeyChecking=no ${REMOTE_SERVER} docker run -d -p ${HOST_PORT}:${CONTAINER_PORT} --name ${CONTAINER_NAME} ${LATEST_IMAGE_TAG}"
                    }
                }
            }
        }
